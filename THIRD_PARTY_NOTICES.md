...
**`talmudic-phrases-community-wikis.json` (2,189 ערכים) — קובץ נפרד, שני
מקורות:**

- 629+13 מכותרות מרחב השם `אנציקלופדיה_תלמודית:` ומהדף "מונחים שכיחים
  בתלמוד" בוויקישיבה (`yeshiva.org.il/wiki`, נשלף ממופע ZIM מקומי) — הצהרת
  רישיון בכל עמוד: **CC BY-NC 3.0** (Attribution-**Noncommercial**).
- 1,563 מכותרות ערכים בוויקיסוגיה (`betmidrash.org.il`, מופע ZIM מקומי) —
  הצהרת הרישיון בכל עמוד: **"רישיון לא ידוע"**.

**⚠️ נקודת עיון, לא הוכרעה כאן:** מה שנלקח מוויקישיבה ומוויקיסוגיה הוא
**רק כותרות ערכים** — מחרוזת קצרה לכל אחד, לא הפרוזה ההסברית שמאחוריה —
ולפי אותו נימוק שחל על `talmudic-phrases.json`/`torah-dictionary.txt`
למעלה. אבל שני המקורות כאן (CC BY-NC, ו"רישיון לא ידוע") שונים מהותית
מהמסגרת הרגילה של המסמך הזה, ושאלת ההתאמה שלהם ל־AGPL-3.0/להפצה דרך חנות
אוצריא לא הוכרעה כאן — בדיוק כפי שהמסמך הזה נמנע במכוון מהכרעות כאלה
במקומות אחרים (ר' לדוגמה איך `@superdoc/docx-engine` תועד: אישור מפורש
מבעלי המקור, לא הנחה). קובץ נפרד מ־`talmudic-phrases.json` בכוונה, בגלל
הפרופיל הרישויי השונה.
